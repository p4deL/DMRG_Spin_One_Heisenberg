...
for alpha, group in grouped:
    ...
    for i in range(num_Lmins):
        ...
        # plot the fit function for the entire data range + append
        if i == 0:
            a_coeffs1.append(a)
    ...
    if alpha > 1.0: # and  alpha < 4.0:
        ...
        popt, pcov = curve_fit(lambda x, a, b: a + b*x, L_scaling[-n_fit_vals:], a_scaling[-n_fit_vals:], p0=[a_scaling[-1],0.05])
...
ax3.plot(alphas, a_coeffs1, 'o-', alpha=0.33, color="C0", label="$L_{\\rm{min}}=$" + f" {int(1./L_scaling[0])}")
ax3.plot(alphas, a_coeffs2, 'o-', alpha=0.66, color="C0", label="$L_{\\rm{min}}=$" + f" {int(1./L_scaling[num_Lmins-1])}")
ax3.errorbar(alphas, a_infinity, yerr=da_infinity, marker="o", color="C0", label="$L_{\\rm{min}}\\rightarrow\\infty$")
