...
output_file_dict = {
    "D" : f"../plots/Gaussian_transition/SvN_mins_alpha{fixed_val}.pdf",
    "alpha" : f"../plots/Gaussian_transition/SvN_mins_D{fixed_val}.pdf"
}
...
if __name__ == "__main__":
    ...
    ax1.scatter(intersections_x, intersections_y, marker='x')
    ...
    ax2.set_xlabel(xlabel, fontsize=fs1)
    ...
    ax3.scatter(inv_Ls, intersections_x, color='#ef476f')
    ax3.set_xlim([0, 0.02])
    ax3.set_xlabel('$1/L$', fontsize=fs1)
    ax3.set_ylabel('$D_c$', fontsize=fs1)
    ...
    y_fit = a + b*x_fit**c
    #y_fit = a - x_fit**b
    ax3.plot(x_fit, y_fit, linestyle='--')
    ...
    ax1.text(0.4, 0.9, f"{fixed_str}$={fixed_val}$", transform=ax1.transAxes, fontsize=12, verticalalignment='center', bbox=props)

    resstr = xlabel + '$_c=%.6f$' % (a, ) + '$\\pm %.6f$' % (da, )
    ax3.text(0.1, 0.2, resstr, transform=ax3.transAxes, fontsize=12, verticalalignment='center', bbox=props)
