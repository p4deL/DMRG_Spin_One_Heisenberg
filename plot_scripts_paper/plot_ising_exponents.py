...
def plot():
    fig = plt.figure(figsize=(6.51, 3.51))
    ...
    ax2.set_position([0.11, 0.25, 0.35, 0.7], which='both')
    ax3.set_position([0.61, 0.25, 0.35, 0.7], which='both')
    #ax3.set_position([0.11, 0.14, 0.35, 0.23], which='both')
    #ax5.set_position([0.61, 0.14, 0.35, 0.23], which='both')

    exp = ("#457b9d", 's', 6, 0.75, "DMRG -- data collapse in $D$")
    ...
    ax2.errorbar(np.reciprocal(alphas), nus, yerr=dnus, color=exp[0], fmt=exp[1], ms=exp[2], alpha=exp[3])
    ...
    ax3.errorbar(np.reciprocal(alphas), betas, yerr=dbetas, color=exp[0], fmt=exp[1], ms=exp[2], alpha=exp[3], label=exp[4])
    ...
    ax2.plot(xnu, ynu, c='lightgray', linewidth=3, zorder=-1)

    xbeta, ybeta = [0.0, 0.36], [0.125, 0.125]
    ax3.plot(xbeta, ybeta, c='lightgray', linewidth=3, zorder=-1)
    #ax2b.plot(xnu, ynu, c='gray', linewidth=2, zorder=-1)

    # plt.tight_layout()
    fig.legend(loc='upper center', bbox_to_anchor=(0.5,0.12), ncols=2, handletextpad=0.2, fontsize=12)
    #fig.legend(loc='lower right',ncol=1, handletextpad=-0.2, columnspacing=0.5, fontsize=fs2)
    fig.savefig("../plots/paper/exponents_ising.pdf")
    plt.show()
