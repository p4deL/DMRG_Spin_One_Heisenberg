...
# output filename
output_file = f"../plots/paper/correlations_L{L}.pdf"
...
corrstr_info = [r'$O^z_{1,j}$', "s", 4, 0.75, '#e07a5f']
corrxx_info = [r'$\langle S_1^xS_j^x \rangle$', "X", 6, 0.75, '#457b9d']
corryy_info = [r'$\langle S_1^yS_j^y \rangle$', "+", 7, 0.75, '#f2cc8f']
corrzz_info = [r'$\langle S_1^zS_j^z \rangle$', ".", 8, 0.75, '#74c69d']
...
fig, axs = plt.subplots(5, 1, figsize=(6, 9), sharex=True)
...
for idx, (ax, filename, label, color) in enumerate(zip(axs, filenames, labels, colors)):
    ...
    ax.text(0.96, 0.8, label + "\\phantom{a}.", transform=ax.transAxes, ha='right', fontsize=fs2, bbox=dict(facecolor='white', alpha=0.5, edgecolor='none', boxstyle='round'))
    ax.scatter(99.7, 0.765, s=120, marker='+', lw=3, color=color, zorder=10)
    ...
    if idx == 0:
        ax.plot(pos, corrstr, label=corrstr_info[0], marker=corrstr_info[1], ms=corrstr_info[2], alpha=corrstr_info[3], color=corrstr_info[4])
        ax.plot(pos, corrxx, label=corrxx_info[0], marker=corrxx_info[1], ms=corrxx_info[2], alpha=corrxx_info[3], color=corrxx_info[4])
        ax.plot(pos, corryy, label=corryy_info[0], marker=corryy_info[1], ms=corryy_info[2], alpha=corryy_info[3], color=corryy_info[4])
        ax.plot(pos, corrzz, label=corrzz_info[0], marker=corrzz_info[1], ms=corrzz_info[2], alpha=corrzz_info[3], color=corrzz_info[4])
    else:
        ax.plot(pos, corrstr, marker=corrstr_info[1], ms=corrstr_info[2], alpha=corrstr_info[3], color=corrstr_info[4])
        ax.plot(pos, corrxx,  marker=corrxx_info[1], ms=corrxx_info[2], alpha=corrxx_info[3], color=corrxx_info[4])
        ax.plot(pos, corryy, marker=corryy_info[1], ms=corryy_info[2], alpha=corryy_info[3], color=corryy_info[4])
        ax.plot(pos, corrzz, marker=corrzz_info[1], ms=corrzz_info[2], alpha=corrzz_info[3], color=corrzz_info[4])
...
fig.legend(lines, labels, bbox_to_anchor=(0.95, 0.87), ncol=4, fontsize='large')
