...
ψfinite = ψ.AR
#@show ψfinite
removetags!(ψfinite, "n=1")
removetags!(ψfinite, "n=2")
println("AFTER:")
for i in 1:10
    c = i÷2
    #println("i=$(i)")
    #println("c=$(c)")
    removetags!(ψfinite, "c=$(c)")
    addtags!(ψfinite[i],"n=$(i)"; tags="Site")
    #removetags!(ψfinite, "n=")
end
#removetags!(ψfinite, "c=1","n=2")
sites = siteinds(ψ[1:2])  # Use the same site indices as the iMPS

sites = siteinds("S=1", 10, conserve_sz=true)
ψfinite = MPS(sites)
for i in 1:10
    ψfinite[i] = ψ[i%2+1]
end
